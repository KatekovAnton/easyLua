...
int MyLoader(lua_State* pState)
{
    ...
    else
    {
        ...
        std::shared_ptr<IFile> file = FileManager::SharedManager()->LoadFile(moduleData.moduleName);
        if(file)
        {
            loaded = true;
...
public:
    ...
    void RetrieveError();
    void LogText(const std::string &text, const std::string &context);
...
void LUAHull::ExecuteCode(const std::string &code)
{
...
void LUAHull::LogText(const std::string &text, const std::string &context)
{
    if (context.length() > 0) {
...
{
    luabridge::LuaRef package = luabridge::getGlobal(_l, "package");
    luabridge::LuaRef loaders = package["loaders"];
...
_dataSource(NULL)
{
    ...
    luabridge::push (LUA, this); // push context to lua stack
...
void LUAContext::ExecuteTestFunctions()
{
    ...
    {
        luabridge::LuaRef function = luabridge::getGlobal (LUA, "subtract");
        luabridge::LuaRef result = function(20, 800);
    ...
    {
        luabridge::LuaRef function = luabridge::getGlobal (LUA, "multiply");
        luabridge::LuaRef result = function(24, 32);
        int resutltInt = result.cast<int>();
        resutltInt = resutltInt;
    }
